...
class Grep         (B): __ror__ = lambda self, it: it | MapSwitch([(lambda x: self.kw.get('i', False), str.lower)]) | Filter(lambda x: re.search(self.f, x))
class GrepV        (B): __ror__ = lambda self, it: it | MapSwitch([(lambda x: self.kw.get('i', False), str.lower)]) | FilterFalse(lambda x: re.search(self.f, x))
