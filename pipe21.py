...
class Grep         (B): __ror__ = lambda self, it: (it | Map(str.lower) if self.kw.get('i', False) else it) | Filter(lambda x: re.search(self.f, x))
class GrepV        (B): __ror__ = lambda self, it: (it | Map(str.lower) if self.kw.get('i', False) else it) | FilterFalse(lambda x: re.search(self.f, x))
